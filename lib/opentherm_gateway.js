...
const write = (id, payload) => {
    console.log(`Write to OpenTherm Gateway => ${id} = ${payload}`)
    ...
    if ( isSupportedMessage(target, type) && id in constants.OPENTHERM_IDS ) {
        ...
        for ( let id in values ) {
            ...
            if ( value !== previousValues[id] ) {
                console.log(`Received from OpenTherm Gateway => ${openthermId.name} = '${value}'`)
                module.exports.emit('message', { field: openthermId.name, value})
